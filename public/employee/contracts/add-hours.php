...
// Resolve current employee (include user_id)
$empStmt = $conn->prepare('SELECT id, name, user_id FROM employees WHERE company_id = ? AND user_id = ? LIMIT 1');
    ...
    elseif ($selected_machine_id > 0 && !in_array($selected_machine_id, $assigned_machine_ids, true)) { $error = __('invalid_machine_selection'); }
        ...
        if ($chk->fetch()) { $error = 'You already have an entry for this date.'; }
            ...
            try {
                ...
                // Notify driver (self) so they can see it in their notifications
                try {
                    $driverUserId = (int)($employee['user_id'] ?? 0);
                    if ($driverUserId > 0) {
                        notifyUser($conn, $driverUserId, 'Hours Logged', 'You logged ' . number_format($hours_worked, 2) . ' hours on ' . $date . ' for contract ' . ($contract['contract_code'] ?? ('#'.$contract_id)) . '.', 'success');
                    }
                } catch (Throwable $nt) {}
