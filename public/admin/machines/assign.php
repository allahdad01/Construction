...
$drivers = [];$assistants = [];
$stmt = $conn->prepare("SELECT id, name, employee_code, user_id FROM employees WHERE company_id = ? AND position = 'driver' AND status='active' ORDER BY name");
$stmt->execute([$company_id]);
$drivers = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
$stmt = $conn->prepare("SELECT id, name, employee_code, user_id FROM employees WHERE company_id = ? AND position = 'driver_assistant' AND status='active' ORDER BY name");
$stmt->execute([$company_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        try {
            ...
            $msg = 'Driver #' . $driver_id . ' assigned to machine ' . ($machine['machine_code'] ?? ('#'.$machine_id)) . ' starting ' . $start_date . '.';
            notifyCompanyUsers($conn, (int)$company_id, $title, $msg, 'info');
        } catch (Throwable $nt) {}
        // Notify driver
        try {
            $st = $conn->prepare('SELECT user_id, name FROM employees WHERE id = ? AND company_id = ?');
            $st->execute([$driver_id, $company_id]);
            $drv = $st->fetch(PDO::FETCH_ASSOC);
            if ($drv && (int)($drv['user_id'] ?? 0) > 0) {
                $title = 'Machine Assignment';
                $msg = 'You have been assigned to machine ' . ($machine['machine_code'] ?? ('#'.$machine_id)) . ' starting ' . $start_date . '.';
                notifyUser($conn, (int)$drv['user_id'], $title, $msg, 'success');
            }
        } catch (Throwable $nt) {}
