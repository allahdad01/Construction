...
if ($contract['contract_type'] === 'hourly') {
    ...
    $total_amount = $total_hours * ($daily_rate / ($contract['working_hours_per_day'] ?: 8));
} elseif ($contract['contract_type'] === 'monthly') {
    $monthly_rate = $contract['rate_amount'];
    $total_amount = $total_hours * ($monthly_rate / ($contract['total_hours_required'] ?: 270));
}
?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong><?php echo __('status'); ?>:</strong> 
                                <span style="font-size: 12px; color: black;" class="badge badge-<?php echo $contract['status'] == 'active' ? 'success' : 'secondary'; ?>">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php 
                                    ...
                                    $hourly_amount = 0;
                                    if ($contract['contract_type'] === 'hourly') {
                                        $hourly_amount = $hours['hours_worked'] * $contract['rate_amount'];
                                    } elseif ($contract['contract_type'] === 'daily') {
                                        $daily_rate = $contract['rate_amount'];
                                        $hourly_amount = $hours['hours_worked'] * ($daily_rate / ($contract['working_hours_per_day'] ?: 8));
                                    } elseif ($contract['contract_type'] === 'monthly') {
                                        $monthly_rate = $contract['rate_amount'];
                                        $hourly_amount = $hours['hours_worked'] * ($monthly_rate / ($contract['total_hours_required'] ?: 270));
                                    }
                                    echo formatCurrencyAmount($hourly_amount, $contract['currency'] ?? 'USD'); 
                                    ?>
                                </td>
