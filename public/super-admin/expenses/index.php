...
// Build query - only super admin expenses (company_id = 0)
$where_conditions = ["company_id = 0"];
$params = [];
...
$stmt = $conn->prepare("SELECT COUNT(*) as total FROM expenses WHERE company_id = 0");
$stmt->execute();
...
$stmt = $conn->prepare("
    SELECT currency, SUM(amount) as total_amount, COUNT(*) as count
    FROM expenses 
    WHERE company_id = 0 AND expense_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)
    GROUP BY currency
");
$stmt->execute();
