...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $vals = [];
        $params = [];
        if (!empty($available['company_id'])) { $cols[] = 'company_id'; $vals[] = 'NULL'; }
...
function generateExpenseCode() {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM expenses");
    $stmt->execute();
    $count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
    
    $next_number = $count + 1;
    return 'EXP' . str_pad($next_number, 4, '0', STR_PAD_LEFT);
}
?>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" id="expense_type" name="expense_type" required>
                                        ...
                                        <option value="maintenance" <?php echo ($_POST['expense_type'] ?? '') === 'maintenance' ? 'selected' : ''; ?>>Maintenance</option>
                                        <option value="marketing" <?php echo ($_POST['expense_type'] ?? '') === 'marketing' ? 'selected' : ''; ?>>Marketing</option>
                                        <option value="software" <?php echo ($_POST['expense_type'] ?? '') === 'software' ? 'selected' : ''; ?>>Software</option>
                                        <option value="travel" <?php echo ($_POST['expense_type'] ?? '') === 'travel' ? 'selected' : ''; ?>>Travel</option>
                                        <option value="other" <?php echo ($_POST['expense_type'] ?? '') === 'other' ? 'selected' : ''; ?>>Other</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
